...
def least_squares(
    ...
    Returns ``(slope, intercept, residual_stddev)``; stddev of residuals is
    the natural width of the confidence band. Degenerate input (fewer than 2
    points, or zero variance in t) yields slope 0.
    """
    n = len(ts)
    if n < 2 or n != len(ys):
...
def forecast_series(
    ...
    *,
    ...
    slope, intercept, stddev = least_squares(ts, ys)
    ...
    eta = saturation_eta(slope, last_value, capacity)
    risk = upper >= capacity and eta is not None and eta <= horizon.total_seconds()
